...
class AuthController extends AControllerRedirect
{
    ...
    public function deleteUser()
    {
        ...
        $this->redirect('auth','logins',['error' => 'Užívateľ vymazaný']);
    }
    ...
    public function newReview()
    {
        ...
        $review = new Review();
        $review->setSender($this->request()->getValue('sender'));
        $review->setReceiver($this->request()->getValue('receiver'));
        $parid = $this->request()->getValue('parid') * 1;

        if (strlen($this->request()->getValue('review')) == 0 || strlen($this->request()->getValue('review')) > 500) {
            $this->redirect('offer', 'singleOffer', ['parid' => $parid, 'error' => 'Nezadali ste žiadne hodnotenie']);
            return;
        }
        $review->setReview($this->request()->getValue('review'));
        ...
        $this->redirect('offer', 'singleOffer', ['parid' => $parid]);
    }
